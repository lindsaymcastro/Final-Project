...
# Add how much prfit was made on each sale
...
```{r}
# Find average sale and retail price and max sale price for each sneaker, arranged by highest sale price to lowest
offwhite %>%
...
sneakers %>%
  ...
  labs(title = "Average Percent of Profit Per Sneaker", x = "Sneaker", y = "Average Percent of Profit") + 
  coord_flip()
```
State that bought the most expensive shoe (not really needed)
State that bought the cheapest shoe (not really needed)
...
```
shoes sold for a loss (dont really need)
